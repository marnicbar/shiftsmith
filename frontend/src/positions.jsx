...
export function Positions({ employees = [], positions, setPositions, groupOrder, setGroupOrder, skills, selId, setSelId, snap, newFlow }) {
  ...
  function createGroup() {
    const name = (prompt(t('positions.newGroupPrompt')) || '').trim();
  ...
  function setPosGroup(id, g) {
    if (g === '__new') { const name = (prompt(t('positions.newGroupPrompt'))||'').trim(); if (!name) return; if (!allGroups.includes(name)) setGroupOrder([...groupOrder, name]); g = name; }
    ...
    <div className="view">
        ...
        <div className="rail-head">
          <div className="row">
            ...
            <div className="splitbtn rail-split">
              <button className="sb-main" onClick={addPosition} title={t('positions.addPosition')}><Ic.plus size={16}/></button>
              <button className={`sb-caret ${menuOpen ? 'on' : ''}`} onClick={() => setMenuOpen((o) => !o)} title={t('common.more')}><Ic.chevD size={13}/></button>
              {menuOpen && (
                <>
                  <div className="menu-backdrop" onClick={() => setMenuOpen(false)}></div>
                  <div className="mini-menu">
                    <button onClick={() => { setMenuOpen(false); addPosition(); }}><Ic.plus size={15}/> {t('positions.menu.newPosition')}</button>
                    <button onClick={() => { setMenuOpen(false); createGroup(); }}><Ic.folderPlus size={15}/> {t('positions.menu.newGroup')}</button>
                  </div>
                </>
              )}
            </div>
              ...
              <div key={g}
                ...
                {isReal ? (
                  <div className={`rail-group-head ${dnd ? 'draggable' : ''}`}
                    ...
                    <span className="muted" style={{ fontSize: 11 }}>{t('positions.groupMeta', { count: items.length, slots: slotsWk })}</span>
                  </div>
                ...
                {isReal && !isCollapsed && !items.length && (
                  <div className="rail-group-empty">{t('positions.dragHere')}</div>
                )}
              </div>
            );
          })}
      ...
      {!pos ? (
        <div className="empty-state">
          <div className="inner">
            <Ic.briefcase/>
            <div style={{ fontSize: 15, fontWeight: 600 }}>{t('positions.emptyTitle')}</div>
            <div className="muted">{t('positions.emptyBody')}</div>
            <button className="btn" onClick={addPosition}><Ic.plus size={15}/> {t('positions.addPosition')}</button>
          </div>
        </div>
      ) : (
      <>
        ...
        <div className="pad">
          ...
          <div className="field">
            ...
            <select className="input" value={pos.group || ''}
              ...
              <option value="__new">{t('positions.newGroupOption')}</option>
            </select>
          </div>
          ...
          <button className="btn danger" style={{ justifyContent: 'center' }}
            onClick={() => { if (confirm(t('positions.confirmDelete', { name: pos.name }))) { const rest = positions.filter((x) => x.id !== pos.id); setPositions(rest); setSelId(rest[0]?.id); } }}>
            <Ic.trash size={15}/> {t('positions.deletePosition')}
          </button>
        </div>
      </div>
      </>
