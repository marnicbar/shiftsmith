...
const OPS = { preferred: 'Preferred', min: 'At least', max: 'At most' };
const isHard = (op) => op !== 'preferred';
...
export function WorkingTimeRules({ emp, onChange }) {
  ...
  function select(r) { setSelId(r.id); setDraft({ metric: r.metric, op: r.op, value: r.value }); setMenu(null); }
  function cancel() { setSelId(null); setDraft(null); setMenu(null); }
  function applyNow() { updateRule(selId, { metric: draft.metric, op: draft.op, value: +draft.value }); cancel(); }
  ...
  function addRule() {
    const r = { id: SS.uid('r'), metric: 'dayHours', op: 'preferred', value: 8, changes: [] };
  ...
  return (
    <div className="field">
      ...
      <div className="hint" style={{ marginTop: 2 }}>Preferred rules are soft goals; "at least" / "at most" are hard limits.</div>
      ...
      <div className="rules">
        ...
        {!rules.length && <div className="hint">No rules yet — add one to constrain this person's hours.</div>}
      </div>
    </div>
  );
}
